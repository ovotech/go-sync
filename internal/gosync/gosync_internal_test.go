...
import (
	"context"
	"errors"
	gosync2 "github.com/ovotech/go-sync/pkg/errors"
	"testing"
	...
	"github.com/ovotech/go-sync/mocks/types"
)

func TestNew(t *testing.T) {
	t.Parallel()

	adapter := types.NewMockAdapter(t)
...
func TestSync_SyncWith(t *testing.T) { //nolint:maintidx
	...
	t.Run("Add", func(t *testing.T) {
		...
		t.Run("Add successful", func(t *testing.T) {
			...
			source := types.NewMockAdapter(t)
			destination := types.NewMockAdapter(t)
	...
	t.Run("MaximumChanges", func(t *testing.T) {
		...
		t.Run("0", func(t *testing.T) {
			...
			require.ErrorIs(t, err, gosync2.ErrTooManyChanges)
		})
